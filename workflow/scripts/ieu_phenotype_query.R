...
option_list <- list(
  ...
  make_option(c("-p", "--pop"), type='character', action='store', default='EUR',
              help = "Population used for downstream analysis, currently population-based filtering not implemented"),
...
get_populated_rows <- function(query) {
  ...
  return(rows)
  #rows_pop <- rows[rowSums(manifest[rows, grep('^n_', grep(pop, headers, value=TRUE), value=TRUE)], na.rm = TRUE) > 0]
  #return(rows_pop)
}
...
write.table(manifest[all_results, ], opt$metadata, row.names=FALSE, sep='\t')
